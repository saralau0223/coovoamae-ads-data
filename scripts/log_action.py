...
def log_action(atype,target,detail):
    today=datetime.utcnow().strftime("%Y-%m-%d");ACTIONS_DIR.mkdir(parents=True,exist_ok=True)
    lf=ACTIONS_DIR/f"{today}.json";actions=[]
    if lf.exists():
    ...
    with open(lf,'w') as f:json.dump({"date":today,"count":len(actions),"actions":actions},f,indent=2,ensure_ascii=False)
